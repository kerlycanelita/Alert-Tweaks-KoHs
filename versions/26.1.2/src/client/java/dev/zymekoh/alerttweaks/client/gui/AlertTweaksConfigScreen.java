...
public final class AlertTweaksConfigScreen extends Screen {
	...
	private void drawPanel(GuiGraphicsExtractor graphics) {
		...
		graphics.outline(this.panelX, this.panelY, this.panelWidth, this.panelHeight, PANEL_BORDER);
	...
	private void drawHeader(GuiGraphicsExtractor graphics) {
		graphics.centeredText(font(), this.title, this.panelX + this.panelWidth / 2, this.panelY + 14, TEXT_MAIN);
	...
	private void drawPreview(GuiGraphicsExtractor graphics) {
		...
		graphics.outline(x - 2, y - 2, width + 4, height + 4, argb(150, 72, 26, 112));
	...
	private void drawPreviewBackground(GuiGraphicsExtractor graphics, int x, int y, int width, int height) {
		if (hasPreviewFrames()) {
	...
	private void drawFallbackPreviewBackground(GuiGraphicsExtractor graphics, int x, int y, int width, int height) {
	...
	private void drawPreviewVignette(GuiGraphicsExtractor graphics, AlertTweaksConfig config, int x, int y, int width, int height) {
	...
	private void drawPreviewThreat(GuiGraphicsExtractor graphics, AlertTweaksConfig config, int x, int y, int width, int height) {
	...
	private void drawSoundPreview(GuiGraphicsExtractor graphics) {
		AlertTweaksConfig config = AlertTweaksConfig.get();
		int x = this.previewX;
		int y = this.previewY;
		int width = this.previewWidth;
		...
		graphics.centeredText(font(), Component.literal("Sound Preview"), centerX, titleY, TEXT_MAIN);
		if (this.previewHeight >= 116) {
			...
			graphics.centeredText(font(), Component.literal("Volume: " + Math.round(config.heartbeatSoundVolume * 100.0F) + "%"),
				centerX, textY, TEXT_MUTED);
			graphics.centeredText(font(), Component.literal("Intensity: " + Math.round(config.heartbeatSoundIntensity * 100.0F) + "%"),
				centerX, textY + 10, TEXT_MUTED);
		} else {
			graphics.centeredText(font(), Component.literal(Math.round(config.heartbeatSoundVolume * 100.0F) + "% volume"),
	...
	private void drawPreviewNote(GuiGraphicsExtractor graphics, AlertTweaksConfig config) {
	...
	private void drawNote(GuiGraphicsExtractor graphics, Component text) {
		...
		graphics.outline(this.previewNoteX, this.previewNoteY, this.previewNoteWidth, this.previewNoteHeight, argb(120, 72, 26, 112));
		...
		for (int i = 0; i < maxLines; i++) {
			graphics.text(font(), lines.get(i), this.previewNoteX + 6, this.previewNoteY + 5 + i * 10, TEXT_MUTED);
		}
	}

	private void drawDescriptionFooter(GuiGraphicsExtractor graphics, int mouseX, int mouseY) {
		...
		graphics.outline(this.descriptionFooterX, this.descriptionFooterY, this.descriptionFooterWidth, this.descriptionFooterHeight,
			argb(180, 72, 26, 112));
		...
		for (int i = 0; i < maxLines; i++) {
			graphics.text(font(), lines.get(i), this.descriptionFooterX + 8, this.descriptionFooterY + 6 + i * 10, TEXT_MUTED);
		}
	}
	...
	private static class ThemeButton extends AbstractWidget implements DescribedControl {
		...
		protected void extractWidgetRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float delta) {
			...
			graphics.outline(this.getX(), this.getY(), this.getWidth(), this.getHeight(), this.isHovered() ? TEXT_ACCENT : PANEL_BORDER);
	...
	private static final class TabButton extends ThemeButton {
		...
		protected void extractWidgetRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float delta) {
			...
			graphics.fill(this.getX(), this.getY(), this.getX() + this.getWidth(), this.getY() + this.getHeight(), color);
			graphics.outline(this.getX(), this.getY(), this.getWidth(), this.getHeight(), this.selected ? TEXT_ACCENT : PANEL_BORDER);
			graphics.text(font(), this.tab.title, this.getX() + 8, this.getY() + 8, this.selected ? TEXT_MAIN : TEXT_MUTED);
			this.handleCursor(graphics);
		}
	}
	...
	private static class ToggleButton extends AbstractWidget implements DescribedControl {
		...
		protected void extractWidgetRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float delta) {
			...
			graphics.text(font(), font().plainSubstrByWidth(this.label, this.getWidth() - 48),
	...
	private static void drawBranch(GuiGraphicsExtractor graphics, int x, int y, int height) {
		int branchX = x - 9;
		int midY = y + height / 2;
	...
	private static class ValueSlider extends AbstractSliderButton implements DescribedControl {
		...
		public void extractWidgetRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float delta) {
			...
			graphics.text(font(), text, this.getX() + 8, this.getY() + 5, TEXT_MAIN);
	...
	private static final class ColorPaletteWidget extends AbstractWidget implements DescribedControl {
		...
		protected void extractWidgetRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float delta) {
			...
			graphics.text(font(), this.getMessage(), this.getX() + 8, this.getY() + 6, TEXT_MAIN);
			...
			graphics.outline(previewX - 1, previewY - 1, 20, 14, argb(160, 230, 210, 255));
		...
		private void drawPalette(GuiGraphicsExtractor graphics, AlertTweaksConfig config) {
			...
			graphics.outline(paletteX - 1, paletteY - 1, paletteWidth + 2, paletteHeight + 2, argb(170, 230, 210, 255));
			...
			graphics.outline(markerX - 3, markerY - 3, 7, 7, argb(255, 255, 255, 255));
			graphics.outline(markerX - 2, markerY - 2, 5, 5, argb(255, 20, 7, 34));
		}
